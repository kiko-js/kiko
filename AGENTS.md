...
- **HMR state is single-instance**: the registry (`hmr.ts`) installs once under `globalThis[Symbol.for("kiko:hmr")]` (`hmr-contract.ts`); `createSignal`'s reuse hook and the `@kikojs/bun` plugin glue both resolve it through that symbol, never through a direct module import — dual-package loads must share one registry. Instance claiming during update batches follows creation-order slots (same heuristic class as hooks order); `realizeLazy` memoizes per lazy object so re-rendered props.children keep node identity.
