...
- **HMR state is single-instance**: the registry (`hmr.ts`) installs once under `globalThis[Symbol.for("kiko:hmr")]` (`contract.ts`); `createSignal`'s reuse hook and the `@kikojs/hmr/bun` plugin glue both resolve it through that symbol, never through a direct module import — dual-package loads must share one registry. The runtime itself is renderer-agnostic: DOM primitives enter through an injected `HmrDomAdapters` (`runtime.ts`); `@kikojs/dom/hmr` is the thin DOM wiring layer (adapters + install). Instance claiming during update batches follows creation-order slots (same heuristic class as hooks order); `realizeLazy` memoizes per lazy object so re-rendered props.children keep node identity. The transport layer stays separate and path-agnostic: `@kikojs/hmr` exposes a Fetch-API hub (`server.ts` `createHmrHub`, SSE built in, any long connection via `HmrSink`) and a pluggable-transport client (`client.ts` `connectHmr`, SSE/WS). The package splits **shared code** from **integrations**: shared = protocol/hub/client/runtime at the root (browser- and SSR-safe, must never pull `oxc-parser` or `node:fs`) plus `transform.ts` (`@kikojs/hmr/transform`) and `watcher.ts` (`@kikojs/hmr/watcher`) behind separate entries; integrations = `bun/` (`@kikojs/hmr/bun` maps a configurable path onto `Bun.serve`'s `fetch`/`websocket` and opts into bundler glue via `transformForHmr(..., { bundler: "bun" })`) and `node/` (`@kikojs/hmr/node`: `createNodeHmr` fetch handler, `toNodeListener` for `node:http`, `bindSocket` for `ws`; no bundler glue). Add a new runtime by adding an integration entry, not by branching inside shared code. The hub never inspects `request.url` paths and module ids remain opaque strings — integrations own routing and path normalization (never hardcode `/hmr` in core). The apply client is likewise a global singleton (`Symbol.for("kiko:hmr:client")`) and must not re-import modules that bundler HMR (`import.meta.hot`) already manages.
...
- Release: manually dispatch "Release PR" workflow → auto-changeset + `changeset version`, opens/updates the `chore: release` PR (gated by CI like any PR) → merge → "Publish" workflow auto-publishes to npm via OIDC trusted publishing (no NPM_TOKEN)
