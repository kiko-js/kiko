...
import { KIKO_HMR, type KikoHmrRegistry } from "./contract"

/**
 * HMR 运行时（`@kikojs/hmr`）。与 `@kikojs/hmr/bun` 插件配合，实现
 ...
 *
 * 本包不依赖任何渲染器：操作 DOM 的原语由宿主经 `HmrDomAdapters` 注入。
 * `@kikojs/dom/hmr` 是 DOM 宿主的接线层（注入 + 安装）；其他渲染器或
 * 未来的 unplugin 适配器可提供自己的适配器。
 */

/** 宿主渲染器向 HMR 运行时注入的 DOM 原语。 */
export interface HmrDomAdapters {
  /** 组件输出 → 顶层节点列表（消解 lazy、拍平 Fragment）。 */
  resolveNodes(out: unknown): Node[]
  /** 递归清理子树上的 watcher/cleanup。 */
  cleanup(node: Node): void
  /** 作用域样式接纳：把节点挂入父级作用域。 */
  adopt(node: Node, parent: Node): void
  /** 组件重跑抛错时上报（避免热更新把异常吞掉）。 */
  report(err: unknown): void
  /** 延迟物化组件输出（kiko 里是 `KikoLazy`：组件体恰好执行一次）。 */
  defer(run: () => unknown): unknown
}

let adapters: HmrDomAdapters | null = null
...
/** 组件实现：`(props) => unknown`，props 不透明透传。 */
...
/** 组件输出 → 顶层节点列表（消解与拍平由宿主适配器完成）。 */
function recordNodes(out: unknown): Node[] {
  return dom().resolveNodes(out)
}
...
function remountInstance(mod: ModuleEntry, inst: Instance): void {
  ...
  if (!parent) {
    ...
    for (const n of next) dom().cleanup(n)
    return
  }
...
const registry: KikoHmrRegistry = {
  ...
  ref(moduleId, name, impl) {
    ...
    return (props: Props) => dom().defer(() => realize(mod, moduleId, name, entry.impl, props))
  },
...
/**
 * 安装（幂等）并返回全局唯一的 HMR 注册表。首次传入宿主适配器完成接线；
 * 后续调用可省略（适配器已就位）。宿主接线层（如 `@kikojs/dom/hmr`）负责传入。
 */
export function installHmr(host?: HmrDomAdapters): KikoHmrRegistry {
  if (host) adapters = host
